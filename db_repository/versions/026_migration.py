...
scrim = Table('scrim', pre_meta,
    ...
    Column('time_zone', VARCHAR(length=80)),
...
scrim = Table('scrim', post_meta,
    Column('id', Integer, primary_key=True, nullable=False),
    Column('day', String(length=80)),
    Column('start_time', String(length=80)),
...
def upgrade(migrate_engine):
    ...
    pre_meta.tables['scrim'].columns['time_zone'].drop()
    post_meta.tables['scrim'].columns['day'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    pre_meta.tables['scrim'].columns['time_zone'].create()
    post_meta.tables['scrim'].columns['day'].drop()
